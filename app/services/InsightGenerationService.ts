...
export const generateUniqueInsights = async (
  ...
  try {
    ...
    if (aiInsights.length > 0) {
      console.log('[Insight Generation] AI insights generated:', aiInsights.length);
      return aiInsights;
    }
 ...
 */
const tryAIGeneration = async (context: InsightGenerationContext): Promise<string[]> => {
 ...
 */
const generateWithGPT2Simple = async (context: InsightGenerationContext): Promise<string[]> => {
  const insights: string[] = [];
  
  // Create simpler, more direct prompts
  const prompts = [
    `Someone is feeling ${context.primaryEmotion}. Give them supportive advice:`,
    `A person wrote "${context.originalText.substring(0, 50)}..." How can they feel better?`,
 ...
 */
const generateWithDialoGPT = async (context: InsightGenerationContext): Promise<string[]> => {
  try {
    ...
    if (response.ok) {
      const data = await response.json();
      if (data && data[0] && data[0].generated_text) {
        const cleaned = cleanAndFormatInsight(data[0].generated_text.replace(prompt, ''));
        if (cleaned.length > 15) {
          return [cleaned];
        }
      }
 ...
 */
const generateWithT5 = async (context: InsightGenerationContext): Promise<string[]> => {
  try {
...
const extractInsightContext = (
  ...
  try {
    ...
    const intensityCount = intensityIndicators.filter(word => lowerText.includes(word)).length;
    const emotionalIntensity = intensityCount > 2 ? 'high' : intensityCount > 0 ? 'medium' : 'low';
 ...
 */
const generateAdvancedContentInsights = (context: InsightGenerationContext): string[] => {
  const insights: string[] = [];
 ...
 */
const generateGuaranteedInsights = (context: InsightGenerationContext): string[] => {
  const insights: string[] = [];
